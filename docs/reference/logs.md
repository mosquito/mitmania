...
Because `outcome` is identical to the metric label, a log-based alert and a metric alert classify a request the same way. Key `outcome` values:
...
_Source: `internal/proxy` (`http1.go`, `http2.go`, `auth.go`)._
